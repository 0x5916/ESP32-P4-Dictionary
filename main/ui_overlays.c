...
/**
 * Wrap text inside brackets () with LVGL recolor tags so brackets and
 * their content render in a dimmer colour and smaller font size.
 * Works in-place on a buffer that must be at least twice the input length.
 */
static void dim_brackets(char *buf, size_t buf_sz, const char *src, const char *hex_dim)
{
    size_t si = 0, di = 0;
    while (src[si] && di < buf_sz - 1) {
        if (src[si] == '(') {
            ...
            if (src[si] == ')') {
                /* Close bracket as literal text, then reset colour */
    ...
    for (uint8_t g = 0; g < entry->pos_count; ++g) {
        ...
        for (uint8_t i = 0; i < group->sense_count; ++i) {
            ...
            if (s->example[0] != '\0') {
                ...
                char ex_buf[520];
                dim_brackets(ex_buf, sizeof(ex_buf), ex_src, hex_dim);
